...
def render_candidate(candidate_folder: Path, title: str, subtitle: str, votos_files: list[Path], bounds_file: Path | None = None):
    ...
    # Definir ordem de prioridade das camadas
    layer_order = ["lider_", "votos_", "popula_", "renda_", "locais_", "distritos"]
    
    def get_layer_priority(layer_name):
        """Retorna prioridade da camada (menor = primeiro)"""
        for i, prefix in enumerate(layer_order):
            if prefix in layer_name:
                return i
        return 999  # Outras camadas por último
    
    # Ordenar camadas por prioridade
    all_layers = sorted(common_layers + cand_layers, key=lambda x: get_layer_priority(x["stem"].lower()))

    for layer in all_layers:
        layer_name = layer["stem"].lower()
        layer_filename = layer["filename"].lower()
        
        # ce_regioes só aparece quando a base for votos_municipios
        if "ce_regioes" in layer_name or "regioes" in layer_name:
            if not is_municipios:
                continue
        ...
        if base_identifier:
            ...
            prefixes = ["votos_", "locais_", "bairros_", "zonas_", "lider_", "renda_", "popula_", "regionais_"]
            
            # Verificar se a camada tem algum prefixo conhecido OU se contém _distritos (ex: quixeramobim_distritos)
            has_prefix = any(layer_name.startswith(p) for p in prefixes)
            has_municipality_suffix = "_distritos" in layer_name or "_bairros" in layer_name
            
            if has_prefix or has_municipality_suffix:
                # Se tem prefixo ou sufixo de município, só mostra se corresponder à base selecionada
                if base_identifier not in layer_name and base_identifier not in layer_filename:
                    continue
        
        # Definir se a camada fica ativa inicialmente
        # Para municípios: ativar ce_regioes e votos_municipios
        if is_municipios:
            show_initially = "ce_regioes" in layer_name or "regioes" in layer_name or "municipios" in layer_name
        else:
            show_initially = "distritos" in layer_name or "locais_" in layer_name
        ...
        # Sobrescrever show com base na regra
        stl["show"] = show_initially
        ...
        if votos_gj:
            ...
            if not df_f.empty:
                ...
                if filtered_features:
                    ...
                    stl = resolve_layer_style(meta, styles)
                    stl["show"] = False  # Camada de votos desativada inicialmente
    ...
    if is_municipios:
        ...
        with g3:
            ...
            if ch_conc is not None:
                st.altair_chart(ch_conc, width="stretch")
        ...
        with g4:
            ...
            if ch_geo is not None:
                st.altair_chart(ch_geo, width="stretch", theme="streamlit")
        ...
        if ch_zona is not None:
            st.altair_chart(ch_zona, width="stretch")
